...
class LockManagerImpl implements LockManager {
	...
	public Locker createLock(final String guid, final int version) {
		try (final Connection conn = this.nextConnection()) {
			...
			{
				try (final PreparedStatement ps = conn.prepareStatement( "SELECT lockId,lockDate,lockExpiration FROM "
						+ this.tableName
						+ " WHERE lockType=? AND lockVersion=?",
					...
					try (final ResultSet rs = ps.executeQuery()) {
						if (rs.next()) {
							return new LockJdbc( this, guid, version, rs.getString( 1 ), false, rs.getTimestamp( 2 )
									.getTime(), rs.getTimestamp( 3 ).getTime() );
						}
						return null;
					}
				}
	...
	public synchronized void start(final String identity) {
		if (this.identity == identity || this.identity.equals( identity )) {
			if (this.lockThread == null) {
				Act.later( null, this.lockThread = new LockThread( this, this.tableName, identity ), 15000L );
