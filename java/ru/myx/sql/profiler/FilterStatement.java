...
class FilterStatement implements Statement {
	...
	public boolean execute(
		...
		try {
			...
			if (this.connection.treshold <= timeTook) {
				this.connection.logger.event( this.owner, "EXECUTE", "time="
	...
	public boolean execute(
			final String sql,
			final int agKeys) throws SQLException {
		...
		try {
			...
			final long timeTook = System.currentTimeMillis() - timeStart;
			if (this.connection.treshold <= timeTook) {
				this.connection.logger.event( this.owner, "EXECUTE_EX", "time="
						+ this.connection.logger.formatPeriod( timeTook )
						+ ", query="
						+ sql );
			}
	...
	public ResultSet executeQuery(
		...
		try {
			...
			if (this.connection.treshold <= timeTook) {
				this.connection.logger.event( this.owner,
						"EXECUTE_QUERY",
						"time=" + this.connection.logger.formatPeriod( timeTook ) + ", query=" + sql );
			}
		}
	}
	...
	public boolean getMoreResults(
			final int current) throws SQLException {
	...
	public void setFetchDirection(
			final int direction) throws SQLException {
