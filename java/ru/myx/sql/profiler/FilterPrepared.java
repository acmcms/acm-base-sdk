...
class FilterPrepared extends FilterStatement implements PreparedStatement {
	...
	FilterPrepared(final String owner,
			final FilterConnection connection,
			final PreparedStatement parent,
			final String sql) {
	...
	public boolean execute() throws SQLException {
		...
		try {
			...
			if (this.connection.treshold <= timeTook) {
				this.connection.logger.event( this.owner,
						"EXECUTE",
						"time=" + this.connection.logger.formatPeriod( timeTook ) + ", query=" + this.sql );
			}
		}
	}
	...
	public int[] executeBatch() throws SQLException {
		...
		try {
			...
			this.connection.logger.event( this.owner, "EXECUTE_BATCH", "error="
					+ e.getMessage()
					+ ", size="
					+ this.batchSize );
			throw e;
			...
			if (this.connection.treshold <= timeTook) {
				this.connection.logger.event( this.owner,
						"EXECUTE_BATCH",
						"time="
								+ this.connection.logger.formatPeriod( timeTook )
								+ ", size="
								+ this.batchSize
								+ ", query="
								+ this.sql );
	...
	public ResultSet executeQuery() throws SQLException {
		...
		try {
			...
			this.connection.logger.event( this.owner, "EXECUTE_QUERY", "error="
					+ e.getMessage()
					+ ", query="
					+ this.sql );
			throw e;
	...
	public int executeUpdate() throws SQLException {
		...
		try {
			...
			this.connection.logger.event( this.owner, "EXECUTE_UPDATE", "error="
					+ e.getMessage()
					+ ", query="
					+ this.sql );
			throw e;
	...
	@Override
	public void setBinaryStream(final int parameterIndex, final InputStream input, final int length)
			throws SQLException {
	...
	@Override
	public void setObject(final int parameterIndex, final Object object, final int targetSqlType, final int scale)
			throws SQLException {
