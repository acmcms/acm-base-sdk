...
public final class Main implements Driver {
	...
	static final int	URL_PREFFIX_LENGTH	= Main.URL_PREFFIX.length();
	static {
		try {
			java.sql.DriverManager.registerDriver( new Main() );
	...
	public boolean acceptsURL(
		...
		return url.startsWith( Main.URL_PREFFIX );
	}
	...
	public final Connection connect(
		...
		if (url.startsWith( Main.URL_PREFFIX )) {
			final String leastUrl = url.substring( Main.URL_PREFFIX_LENGTH );
			...
			final Logger logger = new LoggerStdout( logName );
			...
			final String owner = Main.OWNER + '[' + ++this.index + ']';
	...
	public final DriverPropertyInfo[] getPropertyInfo(
		...
		if (url.startsWith( Main.URL_PREFFIX )) {
			final String realUrl = url.substring( Main.URL_PREFFIX_LENGTH );
