...
public class FileControl {
	...
	public static void main(final String[] args) throws Exception {
		...
		if (suspendFlag.exists()) {
			...
			for (;;) {
				...
				if (!restartFlag.exists()) {
					System.out.println(
							"RU.MYX.AE2CTRL.FILECONTROL: restart request while suspending, sleep time: " + Format.Compact.toPeriod(System.currentTimeMillis() - started) + ".");
					try {
				...
				if (!suspendFlag.exists()) {
					System.out.println("RU.MYX.AE2CTRL.FILECONTROL: suspend cancel, sleep time: " + Format.Compact.toPeriod(System.currentTimeMillis() - started) + ".");
					break;
				}
			}
		}
		...
		Watch.monitor(new FileControlWatcher(restartFlag, suspendFlag, suspendReadyFlag, executeFlag, executeScript, executeOut, executeErr));
	}
	...
	static final void createSuspendReadyFlag(final File suspendReadyFlag) throws Exception {
		...
		try (final FileOutputStream fos = new FileOutputStream(suspendReadyFlag)) {
			fos.write("System is ready to suspend. Create 'suspend' file flag to suspend.".getBytes(StandardCharsets.UTF_8));
		}
	}
