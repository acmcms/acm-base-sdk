...
final class EntryListingContainer extends AbstractContainer<EntryListingContainer> {
	...
	private static final Object STR_SYNC_CANCEL_EXPORT = MultivariantString.getString("Cancel export", Collections.singletonMap("ru", "Отменить экспорт"));
	
	private static final Object STR_SYNC_CANCEL_IMPORT = MultivariantString.getString("Cancel import", Collections.singletonMap("ru", "Отменить импорт"));
	
	private static final Object STR_SYNC_CREATE_EXPORT = MultivariantString.getString("Set export", Collections.singletonMap("ru", "Установить экспорт"));
	
	private static final Object STR_SYNC_CREATE_IMPORT = MultivariantString.getString("Set import", Collections.singletonMap("ru", "Установить импорт"));
			...
			if (entry != null) {
				{
					...
					if (sync != null) {
						...
						if (exportList != null && exportList.length > 0 && Arrays.asList(exportList).contains(key)) {
							result.add(
									Control.createCommand("syncCancelExport", EntryListingContainer.STR_SYNC_CANCEL_EXPORT).setCommandIcon("command-delete")
											.setAttribute("$key", key));
						} else {
							result.add(
									Control.createCommand("syncCreateExport", EntryListingContainer.STR_SYNC_CREATE_EXPORT).setCommandIcon("command-create")
											.setAttribute("$key", key));
						}
						final String[] importList = sync.getImportSynchronizations();
						if (importList != null && importList.length > 0 && Arrays.asList(importList).contains(key)) {
							result.add(
									Control.createCommand("syncCancelImport", EntryListingContainer.STR_SYNC_CANCEL_IMPORT).setCommandIcon("command-delete")
											.setAttribute("$key", key));
						} else {
							result.add(
									Control.createCommand("syncCreateImport", EntryListingContainer.STR_SYNC_CREATE_IMPORT).setCommandIcon("command-create")
											.setAttribute("$key", key));
						}
					}
					final ControlEntry<?> actor = new ControlNodeImpl(this.parent, key);
					{
						...
						if (source != null && !source.isEmpty()) {
							for (final ControlCommand<?> command : source) {
								result.add(
										Control.createCommand(command.getKey(), command.getTitle()).setAttributes(command.getAttributes()).setAttribute("$original", command)
				...
				if (parent != null) {
					...
					if (source != null && !source.isEmpty()) {
						for (final ControlCommand<?> command : source) {
							result.add(
									Control.createCommand("cntn" + command.getKey(), command.getTitle()).setAttributes(command.getAttributes()).setAttribute("$original", command)
											.setAttribute("$actor", actor));
						}
					}
				}
