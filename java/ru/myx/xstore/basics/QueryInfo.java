...
final class QueryInfo {
	...
	private static final ReplyAnswer handleFieldRequest(
		...
		if (!respondable.contains( field )) {
			return Reply.string( QueryInfo.OWNER, //
					query,
		...
		try {
			...
			if (objectImpl == BaseObject.UNDEFINED) {
				return Reply.string( QueryInfo.OWNER, query, "Not found: " + entry.getLocationControl() + ":" + field )
						.setCode( Reply.CD_UNKNOWN ).setFlags( flags );
			}
			...
			if (object == null) {
				...
				if (bytes.length <= Transfer.BUFFER_MEDIUM) {
					response = Reply.wrap( QueryInfo.OWNER, query, bytes, contentName ).setContentType( contentType )
							.setTimeToLiveDays( 1 ).setFinal();
				} else {
					...
					response = Reply.binary( QueryInfo.OWNER, query, coll.toBinary(), contentName )
							.setContentType( contentType ).setTimeToLiveDays( 1 ).setFinal();
				}
				...
				response = Reply.binary( QueryInfo.OWNER, query, buffer, contentName ).setContentType( contentType )
						.setTimeToLiveDays( 1 ).setFinal();
				...
				response = Reply.binary( QueryInfo.OWNER, query, copier, contentName ).setContentType( contentType )
						.setTimeToLiveDays( 1 ).setFinal();
				...
				if (Base.getString( response.getAttributes(), "Content-Name", "" ).trim().length() == 0) {
				...
				contentName = Base.getString( query.getParameters(), "name", entry.getKey() );
				response = Reply.string( QueryInfo.OWNER, query, object.toString() ).setContentName( contentName )
						.setFlags( flags );
			}
	...
	final ReplyAnswer handleQuery(final StorageImpl parent, final ServeRequest query) {
		...
		case TP_QUERY: {
			...
			if ("POST".equals( query.getVerb() )) {
				final AccessPrincipal<?> principal = parent
						.getServer()
						.getAccessManager()
						.securityCheck( AuthLevels.AL_AUTHORIZED_NORMAL,
								targetEntry.getLocationControl(),
								this.containsDraft
										? "execute_draft"
										: "execute" );
				if (principal == null) {
					Report.audit( QueryInfo.OWNER, //
							"DENIED",
							"Security check failed: path="
									+ targetEntry.getLocationControl()
									+ ", principal="
									+ principal );
				...
				if (principal.isPerson()
						|| ((AccessGroup<?>) principal).getAuthLevel() >= AuthLevels.AL_AUTHORIZED_AUTOMATICALLY) {
					Report.audit( QueryInfo.OWNER, //
							"GRANTED",
							"Security check granted execution: path="
									+ targetEntry.getLocationControl()
									+ ", principal="
									+ principal );
				}
				...
				if (principal.isPerson()
					...
					Report.audit( QueryInfo.OWNER, //
							"GRANTED",
							"Security check granted elevation: path="
									+ targetEntry.getLocationControl()
									+ ", principal="
									+ principal );
					privateResponse = true;
							...
							try {
								...
								if (object == null) {
									...
									if (this.filters.isEmpty()) {
										response = Reply.object( response.getEventTypeId(),
												query,
			...
			if (cache != null) {
				...
				if (Base.getString( response.getAttributes(), "Content-Disposition", "" ).trim().length() == 0) {
			...
			return clientTtl > 0
					? response.setTimeToLiveSeconds( clientTtl )
					: Base.getString( response.getAttributes(), "Content-Disposition", "" ).trim().length() == 0
							? response.setNoCaching()
							: response;
		}
		case TP_FIELD: {
		...
		case TP_REDIRECT: {
			return Reply.redirect( QueryInfo.OWNER,
					query,
					true,
					Context.getServer( Exec.currentProcess() ).fixUrl( this.paramString ) ).setTimeToLiveMinutes( 30 );
		}
		case TP_ERROR: {
			return Reply.string( QueryInfo.OWNER, query, this.paramString ).setCode( Reply.CD_EXCEPTION )
					.setNoCaching().setPrivate().setFlags( Context.getContext( Exec.currentProcess() ).getFlags() );
		}
		case TP_LAST_ANY: {
		...
		default: {
			return Reply.string( QueryInfo.OWNER, query, "UNKNOWN RESPONSE!" ).setCode( Reply.CD_EXCEPTION )
					.setNoCaching().setPrivate().setFlags( Context.getContext( Exec.currentProcess() ).getFlags() );
		}
		}
	}
